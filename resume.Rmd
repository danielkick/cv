...
- `r icons::fontawesome$solid$envelope` [<font size="-2">hello@danielkick.com</font>](hello@danielkick.com)
- `r icons::fontawesome$brands$linkedin` [<font size="-2">daniel-kick-5a449b9a</font>](https://www.linkedin.com/in/daniel-kick-5a449b9a/)
- `r icons::academicons$"google-scholar"` [<font size="-2">Google Scholar</font>](https://scholar.google.com/citations?user=5Sz0E-0AAAAJ&hl=en&oi=ao)
<!-- - `r icons::fontawesome$brands$orcid` [0000-0002-9002-1862](https://orcid.org/0000-0002-9002-1862) -->
- `r icons::fontawesome$brands$github` [<font size="-2">github.com/DanielKick-USDA</font>](https://github.com/DanielKick-USDA)
- `r icons::fontawesome$brands$github` [<font size="-2">github.com/danielkick</font>](https://github.com/danielkick)
- `r icons::fontawesome$solid$link` [<font size="-2">danielkick.com</font>](https://www.danielkick.com) 
...
<font size="-2">Next Generation Sequence Data and Analysis, Bioinformatics, **Leadership role** in the biological honors society Tri-Beta</font>
...
### Technical Skills
 ...
 `r icons::fontawesome$brands$python`: **Python **  (`r round(as.numeric(difftime(as.Date(format(Sys.time(), "%Y-%m-%d")), as.Date("2021-06-09"), units = "weeks"))/52, 0)` years) experience with <span style="color:DarkRed;">`pandas`, `numpy`, `plotly`, `scikit-learn`, `keras`, `pytorch`</span>.
 
 `r icons::fontawesome$brands$"r-project"`: **R **  (`r round(as.numeric(difftime(as.Date(format(Sys.time(), "%Y-%m-%d")), as.Date("2017-01-29"), units = "weeks"))/52, 0)` years) experience with <span style="color: DarkRed;">`tidyverse`, `lme4`, `caret`, `ggplot2`, `shiny`</span>, & package creation. 
 
 `r icons::octicons$terminal`: **Miscellaneous **
 Experience with high performance computing (<span style="color:DarkRed;">`bash`, `slurm`</span>), virtual environments (<span style="color:DarkRed;">`conda`, `singularity`</span>), version control (<span style="color:DarkRed;">`git`, `GitHub`</span>), literate programming (<span style="color:DarkRed;">`Quarto`, `Rmarkdown`, `Jupyter`, `nbdev`</span>), Scientific modeling (crop growth modeling <span style="color:DarkRed;">`APSIM Next Generation`</span>), basic <span style="color:DarkRed;">`SQL`</span> (<span style="color:DarkRed;">`SQLite`)</span>.
...
- Experience predicting maize yield across environments using **deep learning, machine learning, statistical modeling** to facilitate crop improvement.
- **Designed funded research project** (USDA NIFA) to create environmentally aware **deep learning models for trait prediction** and **transfer learning** between maize and wheat ($225,000, [Grant 2023-67012-39485](https://portal.nifa.usda.gov/web/crisprojectpages/1030152-environmentally-aware-deep-learning-based-genomic-selection-and-management-optimization-for-maize-yield.html)).
- Initiated and led a multi-institutional research collaboration; collaborated with domain experts.
...
- **Developed data analysis app** used by **>700 students** as of 2021.
...
- **Led 4 teaching assistants** and **mentored `r postdoc_mentoring+phd_mentoring` research students**.
...
- **Initiated and led a multi-institutional research collaboration** and wrote custom libraries for same.
- Employed **deep neural networks, machine learning models, best linear unbiased predictors, and scientific models (i.e. process based)** to improve trait prediction accuracy in diverse environments.
...
- **Organized and lead** deep learning community of practice university and government attendees. 
- **Mentored** `r postdoc_mentoring` students with projects ranging from high throughput phenotyping experiment, to deep learning model development. 
...
- Designed and completed a professional development curriculum with the guidance of an industry scientists via the **Bayer-University Mentoring Program** and the **Maize Genetics Mentoring Program**.
...
- Iteratively refined curriculum, modeled student grade distributions to identify and adjust for differences in grading.
...
- Redesigned and updated course material to incorporate data analysis and primary literature.
...
Honors and Awards (Showing `r resume_frac(get_section = 'honors')`) {data-icon=award}
--------------------------------------------------------------------------------
```{r}
...
<!-- Selected Publications (Showing `r resume_frac(get_section = 'academic_articles')` Published, with `r resume_frac(get_section = 'academic_articles_prep', half = T)` in Prep.) {data-icon=flask} -->


Selected Publications (Showing `r resume_frac(get_section = 'academic_articles')` Published) {data-icon=flask}
--------------------------------------------------------------------------------
```{r}
